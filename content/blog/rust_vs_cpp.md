...
> There's a common pattern I see all too often in Rust code, which I call a "pointer with extra steps". It's when you have integer "IDs" that are used to index into an array or a hashmap. This essentially recreates the functionality of a pointer without having Rust's compiler complain at you. In practice this is a roundabout way of doing things, and more often than not the performance will be slower than if you just used the `Rc<RefCell<T>>` (or the thread-safe equivalent `Arc<Mutex<T>>`) smart pointers in Rust. But of course since `Rc<RefCell<T>>` is cumbersome to type and indexing into an array or a hashmap is not, people tend to choose the latter.
